...
class _AdminPanelState extends State<AdminPanel> {
  ...
  Future<bool> uploadFile(
    ...
    if (result != null) {
      ...
      final String fileName = '${crop}_$region.pdf';

      if (fileBytes == null) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Unable to read the selected PDF')),
          );
        }
        return false;
      }
      ...
      try {
        ...
        if (context.mounted) {
          final message = e is FirebaseException
              ? 'Firebase ${e.code}: ${e.message ?? 'operation failed'}'
                                ...
                                TextButton(
                                  onPressed: () async {
                                    if (selectedCrop != null &&
                                        selectedRegion != null) {
                                      final uploaded = await uploadFile(context,
